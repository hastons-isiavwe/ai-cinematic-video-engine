...
for i, img in enumerate(image_paths):
    ...
    if motion == "zoom_in":
        clip = clip.resize(lambda t: 1.00 + 0.15 * (t / shot_duration))

    elif motion == "zoom_out":
        clip = clip.resize(lambda t: 1.15 - 0.15 * (t / shot_duration))

    elif motion == "pan_left":
        clip = clip.resize(1.22).set_position(
            lambda t: (-140 * (t / shot_duration), "center")
        )

    elif motion == "pan_right":
        clip = clip.resize(1.22).set_position(
            lambda t: (-140 + 140 * (t / shot_duration), "center")
        )

    else:
        # even static shots get subtle movement
        clip = clip.resize(lambda t: 1.00 + 0.08 * (t / shot_duration))
