...
for i, img in enumerate(image_paths):
    ...
    # -----------------------------
    # ADVANCED CINEMATIC MOTION FX
    # -----------------------------
    if motion == "zoom_in":
        clip = clip.resize(
            lambda t: 1.00 + 0.18 * (t / shot_duration)
        )

    elif motion == "zoom_out":
        clip = clip.resize(
            lambda t: 1.18 - 0.18 * (t / shot_duration)
        )
        ...
        clip = (
            clip.resize(
                lambda t: 1.03 + 0.05 * (t / shot_duration)
            )
            .set_position(
                lambda t: (
                    -20 + 40 * (t / shot_duration),
                    -10
                )
            )
        )
        ...
        clip = clip.resize(
            lambda t: 1.00 + 0.06 * (t / shot_duration)
        )
