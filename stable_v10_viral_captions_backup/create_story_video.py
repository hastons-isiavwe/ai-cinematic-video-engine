...
# -----------------------------
def create_word_caption(word, active=False):
...
# CAPTION MODE SYSTEM
# -----------------------------
from config.settings import CAPTION_MODE
...
if CAPTION_MODE == "FAST":
    ...
    def create_caption(text, active=True):
        img = Image.new("RGBA", (OUTPUT_WIDTH, 200), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
    ...
    for i, text in enumerate(groups):
        ...
        img = create_caption(text, active=True)
